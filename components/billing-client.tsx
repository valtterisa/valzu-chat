...
export function BillingClient() {
  ...
  const tokenUsage = customer?.features?.token_usage;
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-3xl flex-col gap-6 px-4 py-8">
        ...
        <p className="text-sm">
          {currentProduct ? (
            <>
              ...
              {tokenUsage?.included_usage && tokenUsage?.interval && (
                <>
                  {" "}
                  · {tokenUsage.included_usage} messages / {tokenUsage.interval}
                </>
              )}
            </>
          ) : (
      ...
      <section className="rounded-xl border border-border bg-card p-4 sm:p-6">
        ...
        {tokenUsage ? (
          <p className="text-sm">
            Balance: <span className="font-medium">{tokenUsage.balance}</span> /{" "}
            <span className="font-medium">{tokenUsage.included_usage}</span>{" "}
            messages this {tokenUsage.interval}
          </p>
        ) : (
          <p className="text-sm text-muted-foreground">
